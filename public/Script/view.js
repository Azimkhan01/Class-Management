...
async function totalAttendance() {
  try {
    ...
    let newresponse = await fetch(
      `${window.location.origin}/getStudents?_id=${paramsId}`
    );
    ...
    currentBatch.forEach((attendanceRecord) => {
      ...
      if (!addedMonths.includes(month)) {
        ...
        monthHeader.innerText = months[month].toUpperCase() + "-" + year;
        ...
        for (let j = 1; j <= days[month]; j++) {
          ...
          dayDiv.innerHTML = `<abbr title="${dayDiv.id}">${j}</abbr>`; // Display the day number
    ...
    studentData[0].present.forEach((e) => {
      ...
      d.style.backgroundColor = "#9DC610";
    });
